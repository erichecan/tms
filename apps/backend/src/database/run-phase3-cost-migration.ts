...
async function runPhase3CostMigration() {
  ...
  try {
    console.log('开始执行第三阶段成本核算数据库迁移...');

    const projectRoot = path.resolve(__dirname, '../../../..');
    const migrationFile = path.join(projectRoot, 'database_migrations/014_cost_accounting.sql');
