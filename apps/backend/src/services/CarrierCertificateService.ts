...
export interface CarrierCertificate {
  ...
  carrierId: string;
  tenantId: string;
  certificateType: 'business_license' | 'transport_permit' | 'hazardous_permit' | 'cold_chain' | 'insurance' | 'other';
...
export interface CreateCarrierCertificateInput {
  carrierId: string;
  certificateType: 'business_license' | 'transport_permit' | 'hazardous_permit' | 'cold_chain' | 'insurance' | 'other';
  certificateNumber: string;
  issueDate?: Date | string;
...
export interface UpdateCarrierCertificateInput {
  certificateType?: 'business_license' | 'transport_permit' | 'hazardous_permit' | 'cold_chain' | 'insurance' | 'other';
  certificateNumber?: string;
  issueDate?: Date | string;
  expiryDate?: Date | string;
...
export class CarrierCertificateService {
  ...
  async createCertificate(
    tenantId: string,
    input: CreateCarrierCertificateInput
  ): Promise<CarrierCertificate> {
    try {
      const query = `
        INSERT INTO carrier_certificates (
          carrier_id, tenant_id, certificate_type, certificate_number,
      ...
      logger.error('Failed to create carrier certificate:', error);
      throw error;
    }
  }
   ...
   * 获取承运商的所有证照
   */
  async getCertificatesByCarrier(
    tenantId: string,
    carrierId: string
  ): Promise<CarrierCertificate[]> {
    try {
      const query = `
        SELECT * FROM carrier_certificates
      ...
      logger.error('Failed to get carrier certificates:', error);
      throw error;
    }
  }
  ...
  async getExpiringCertificates(
    ...
    try {
      const query = `
        SELECT cc.*, c.name as carrier_name
        FROM carrier_certificates cc
        JOIN carriers c ON cc.carrier_id = c.id
        WHERE cc.tenant_id = $1
          AND cc.expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '${daysAhead} days'
  ...
  async updateCertificate(
    ...
    input: UpdateCarrierCertificateInput
  ): Promise<CarrierCertificate> {
    try {
      ...
      logger.error('Failed to update carrier certificate:', error);
      throw error;
    }
  }
  ...
  async deleteCertificate(
    ...
    try {
      const query = `
        DELETE FROM carrier_certificates
      ...
      logger.error('Failed to delete carrier certificate:', error);
      throw error;
    }
  }
   ...
   * 映射数据库记录到接口
   */
  private mapCertificateFromDb(row: any): CarrierCertificate {
    return {
      id: row.id,
