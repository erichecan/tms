...
import { Truck, Clock, FileText, CheckCircle2, ArrowLeft, ArrowRight, X } from 'lucide-react';
    ...
    if (!metrics) return (
        <div style={{ display: 'flex', height: '60vh', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ color: 'var(--slate-500)', fontWeight: 800, fontSize: '13px', textTransform: 'uppercase' }}>{t('dashboard.syncData')}</div>
            </div>
        </div>
    );
            ...
            <div className="glass" style={{ padding: '0', overflow: 'hidden' }}>
                <div style={{ padding: '32px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <p style={{ margin: '4px 0 0', color: 'var(--slate-500)', fontSize: '14px' }}>{t('dashboard.pendingWaybillsSubtitle')}</p>
                    </div>
                    <button className="btn-secondary" style={{ padding: '10px 24px' }}>{t('dashboard.analyzeReports')}</button>
                </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0' }}>
                        <thead>
                            <tr style={{ background: 'var(--slate-50)' }}>
                                ...
                                <th style={{ padding: '16px 24px', color: 'var(--slate-400)', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase' }}>{t('dashboard.table.route')}</th>
                                <th style={{ padding: '16px 24px', color: 'var(--slate-400)', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase' }}>{t('dashboard.table.status')}</th>
                                <th style={{ padding: '16px 24px', color: 'var(--slate-400)', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase' }}>{t('dashboard.table.valuation')}</th>
                                <th style={{ padding: '16px 24px', textAlign: 'right' }}></th>
                            </tr>
                        </thead>
                        <tbody>
                            {jobs.length === 0 ? (
                                <tr><td colSpan={5} style={{ padding: '60px', textAlign: 'center', color: 'var(--slate-400)', fontWeight: 600 }}>{t('dashboard.table.empty')}</td></tr>
                                ...
                                <tr key={job.id} style={{ borderBottom: '1px solid var(--glass-border)' }} className="table-row-hover">
                                    ...
                                    <td style={{ padding: '20px 24px', textAlign: 'right' }}>
                                        {job.status === 'NEW' ? (
                                            <button onClick={() => openAssignModal(job.id)} className="btn-primary" style={{ padding: '10px 24px', fontSize: '12px' }}>{t('dashboard.table.dispatch')}</button>
                                        ) : (
                                            <button onClick={() => navigate(`/tracking/${job.trip_id || job.id}`)} className="btn-secondary" style={{ padding: '10px 24px', fontSize: '12px' }}>{t('dashboard.table.track')}</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, animation: 'fadeIn 0.3s ease-out' }}>
                    <div className="glass-card" style={{ width: '480px', padding: '40px', background: 'white' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                            ...
                            <button onClick={() => setIsModalOpen(false)} style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--slate-400)', padding: '4px' }}>
                                <X size={24} />
                            </button>
                        </div>
