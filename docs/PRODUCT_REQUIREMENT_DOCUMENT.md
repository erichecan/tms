...
**版本:** 2.1 (结构调整，去除即时“报价/确认”流程，扩展运单创建字段，并为未来智能报价预留数据结构)  
**最后更新:** 2025-09-23  
...
变更点 | 说明
-------|-----
去除“智能报价”在线计算环节 | 本迭代不实现实时计价流程；未来模块保留数据与接口预留
去除“pending / quoted / confirmed”多级确认 | 运单创建即视为已确认（满足录单或客户已同意），直接进入调度
新增运单创建字段 | 参考常见国际/电商物流创建界面，补齐收发件、包裹、商品、估值/报关、标签等
新增状态机简化 | 流程：created → (unassigned) → assigned → picked_up → in_transit → delivered → completed
预留价格/规则字段 | estimatedCost, pricingComponents[], pricingRuleTrace[] 作为后续智能定价使用
新增包裹与商品明细实体 | 拆分 shipment_package / shipment_item
财务触发点不变 | 仍在 completed 时生成应收/应付
规则引擎说明调整 | 当前仅预留 pricing 类型，不执行；未来 payroll / pricing 两类
补充字段验证与单位规范 | 货物重量、体积、尺寸、货值、币种等
