...
**版本:** 3.2.0 (Google Maps集成与智能调度优化版本)  
...
变更点 | 说明
-------|-----
**🗺️ Google Maps深度集成** | 地址自动完成、实时路径计算、地图可视化、基于实际距离的智能计费
**🤖 智能调度系统优化** | 贪心算法(<50单)+遗传算法(≥50单)混合策略，批量选择，实时优化
**✏️ 运单编辑功能增强** | 支持在线编辑发货人/收货人/地址/货物信息，权限控制
**💰 财务结算页面简化** | 简洁的财务概览，Tab切换（应收/应付/报表），按客户/司机分组
**📊 司机薪酬页面优化** | 核心指标卡片（任务数/总收入/准时率/POD上传率），简洁明细表格
**🔧 Google Maps Loader优化** | 修复重复初始化错误，真正的单例模式
**🎯 计费引擎调用修复** | 确保使用后端真实计费引擎，不降级到本地计算
**📍 地图中心点统一** | 所有地图默认中心：3401 Dufferin St, North York, ON M6A 2T9
